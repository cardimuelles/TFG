...
def splitShuffleData(matrix,num_step,trainTestRatio,maxSize,features):
    ...
    inputShuffled = np.float32(np.array(shuffleMatrix(reshapedInput)).reshape(maxSize-num_step,num_step,features))
    outputShuffled = np.float32(np.array(shuffleMatrix(reshapedOutput)).reshape(maxSize-num_step,1,features))
